<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['disposisi_kpd'] == "" || $_REQUEST['petunjuk_disposisi'] == "" || $_REQUEST['sifat'] == "" || $_REQUEST['batas_waktu'] == ""
                || $_REQUEST['catatan'] == ""){
                $_SESSION['errEmpty'] = 'ERROR! Semua form wajib diisi';
                echo '<script language="javascript">window.history.back();</script>';
            } else {

                $disposisi_kpd = $_REQUEST['disposisi_kpd'];
                $petunjuk_disposisi = $_REQUEST['petunjuk_disposisi'];
                ...
                //validasi input data
                if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $disposisi_kpd)){
                    $_SESSION['disposisi_kpd'] = 'Form Disposisi Kepada hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,) minus(-). kurung() dan garis miring(/)';
                    echo '<script language="javascript">window.history.back();</script>';
                } else {

                    if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $petunjuk_disposisi)){
                        $_SESSION['petunjuk_disposisi'] = 'Form Petunjuk Disposisi hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan(&), underscore(_), kurung(), persen(%) dan at(@)';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {
                        ...
                        if(!preg_match("/^[0-9 -]*$/", $batas_waktu)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,()%@\/ -]*$/", $catatan)){
                                ...
                                if(!preg_match("/^[a-zA-Z0 ]*$/", $sifat)){
                                    ...
                                    $query = mysqli_query($config, "INSERT INTO tbl_disposisi(disposisi_kpd,petunjuk_disposisi,sifat,batas_waktu,catatan,id_surat,id_user)
                                        VALUES('$disposisi_kpd','$petunjuk_disposisi','$sifat','$batas_waktu','$catatan','$id_surat','$id_user')");
                ...
                <form class="col s12" method="post" action="">
                    ...
                    <div class="row">
                        <div class="input-field col s6">
                            ...
                            <input id="disposisi_kpd" type="checkbox" class="validate" name="disposisi_kpd" required>
                                <?php
                                    if(isset($_SESSION['disposisi_kpd'])){
                                        $disposisi_kpd = $_SESSION['disposisi_kpd'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$disposisi_kpd.'</div>';
                                        unset($_SESSION['disposisi_kpd']);
                                    }
                                ?>
                            <label for="disposisi_kpd">Di Kepada</label>
                        </div>
                        <div class="input-field col s6">
                        ...
                        <div class="input-field col s6">
                            ...
                            <textarea id="petunjuk_disposisi" class="materialize-textarea validate" name="petunjuk_disposisi" required></textarea>
                                <?php
                                    if(isset($_SESSION['petunjuk_disposisi'])){
                                        $petunjuk_disposisi = $_SESSION['petunjuk_disposisi'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$petunjuk_disposisi.'</div>';
                                        unset($_SESSION['petunjuk_disposisi']);
                                    }
                                ?>
                            <label for="petunjuk_disposisi">Petunjuk Disposisi</label>
                        </div>
                        <div class="input-field col s6">
